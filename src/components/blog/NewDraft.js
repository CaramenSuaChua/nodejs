...
const NewDraft = () => {
  ...
  <>
        <div className="d-flex box-top justify-content-between" style={{marginTop:'30px',}}>
          ...
          <button type="button" class="btn btn-outline-success" style={{marginRight:'65px'}}>Add NewDraft</button>
        </div>
        ...
        {/* <tr> */}
          {data.length > 0 && data.map((h, idx) => {
            return(
              ...
              <td>{h.payment}</td>
              <td style={{}}>
              <span className={h.status === 'Booked' ? 'statusBooked' : (h.status === 'Checkin' ? 'statusCheckin' : 'statusCheckout')}>{h.status}</span></td>
              </tr>
            )
          }).splice(0,8)}
        {/* </tr> */}
